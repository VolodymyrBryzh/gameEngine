...
impl Camera {
    fn update_position(&mut self, target: Vec3) {
        self.target = target;
        let horizontal_dist = self.distance * self.pitch.to_radians().cos();
        let vertical_dist = self.distance * self.pitch.to_radians().sin();

        let x_offset = horizontal_dist * self.yaw.to_radians().sin();
        let z_offset = horizontal_dist * self.yaw.to_radians().cos();

        self.eye = Vec3::new(target.x - x_offset, target.y + vertical_dist, target.z - z_offset);
    }
...
impl Engine {
    pub async fn new(window: winit::window::Window) -> Self {
        ...
        let camera_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: None, contents: bytemuck::cast_slice(&[camera_uniform]), usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        });
        ...
        let model_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: None, contents: bytemuck::cast_slice(&[model_uniform]), usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        });
        ...
        for z in 0..terrain_res {
            for x in 0..terrain_res {
                ...
                let h = perlin.get([fx as f64 * 0.03, fz as f64 * 0.03]) as f32 * 6.0;
                heights[(z, x)] = h;
                let color = if h > 2.0 { [0.5, 0.5, 0.5] } else { [0.2, 0.5, 0.2] };
        ...
        let terrain_collider = ColliderBuilder::heightfield(heights, vector![terrain_size as f32, 1.0, terrain_size as f32]).build();
        ...
        Self {
            ...
            render_pipeline, cube_vertex_buffer, cube_index_buffer, num_cube_indices: cube_indices.len() as u32,
    ...
    pub fn update(&mut self) {
        ...
        // Оновлення обертання камери мишею
        self.camera.yaw += self.mouse_delta.0 * 0.1;
        self.camera.pitch = (self.camera.pitch - self.mouse_delta.1 * 0.1).clamp(5.0, 85.0);
        ...
        for (_id, (handle, _player)) in self.world.query_mut::<(&PhysicsHandle, &Player)>() {
            ...
            if move_dir.length() > 0.0 {
                let vel = move_dir.normalize() * 30.0;
                body.set_linvel(vector![vel.x, body.linvel().y, vel.z], true);
                body.wake_up(true);
            } else {
                body.set_linvel(vector![0.0, body.linvel().y, 0.0], true);
            }

            if self.input.contains(&KeyCode::Space) && body.linvel().y.abs() < 0.1 {
                body.apply_impulse(vector![0.0, 15.0, 0.0], true);
            }

            self.camera.update_position(Vec3::new(pos.x, pos.y, pos.z));
            self.window.set_title(&format!("Survival Engine - Pos: X:{:.1} Z:{:.1}", pos.x, pos.z));
        }
...
pub async fn run() {
    ...
    // Захоплення курсора (необов'язково, але зручно для FPS керування)
    ...
    let _ = event_loop.run(move |event, elwt| {
        match event {
            Event::DeviceEvent { event: DeviceEvent::MouseMotion { delta }, .. } => {
                engine.mouse_delta.0 += delta.0 as f32;
                engine.mouse_delta.1 += delta.1 as f32;
            }
